...
if [ "$STATUS" = RUNNING ]; then
    gcloud compute instances get-serial-port-output "$NAME" --zone="$ZONE" 2>/dev/null | grep -aE "$WANTED" | tail -n 12
else
    ...
    gcloud logging read "resource.type=gce_instance AND resource.labels.instance_id=$ID AND logName:serial_port_1_output" \
        --freshness=7d --limit=400 --format="value(textPayload)" | grep -aE "$WANTED" | head -n 12 | tac
